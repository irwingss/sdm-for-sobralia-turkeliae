# ============================================================================
# Notebook: nod_reproducibility-rehabilitation
# Language: R
# Key: nod_reproducibility-rehabilitation
# Generated: 2026-08-06T18:10:13.082Z
# Auto-generated by Ressearch AI — reproducible code trail.
# ============================================================================

# ── Environment setup ─────────────────────────────
# Instala dependencias detectadas (una sola vez).
# install.packages(c("dplyr", "ENMeval", "ggnewscale", "ggplot2", "ggrepel", "jsonlite", "maxnet", "pROC", "readxl", "sf", "spThin", "terra", "tidyr", "tidyterra"))

.requirements <- c("dplyr", "ENMeval", "ggnewscale", "ggplot2", "ggrepel", "jsonlite", "maxnet", "pROC", "readxl", "sf", "spThin", "terra", "tidyr", "tidyterra")
...
# ──── #1 · Ejecución · 2026-08-06T15:22:44.488Z ───────────────────────────────────
# Rerun entire SDM pipeline reproducibly
# Propósito: Execute the fully rehabilitated end-to-end SDM pipeline for Sobralia turkeliae with 100% determinism.
# Artifacts: #3905, #3907, #3906, #3908, #3909
# runtime: modal:sdm-r
# duration_ms: 18296
...
set.seed(42) # Establish global determinism for all sampling

# -----------------------------------------------------------------------------
# STEP 1: Prepare M Area and Generate Variable Dendrogram
# -----------------------------------------------------------------------------
message("[1/10] Loading and cleaning occurrences...")
df <- readxl::read_excel("Coordenadas_Sobralia turkeliae_nov.xlsx")
df_clean <- df %>%
  filter(!is.na(lon) & !is.na(lat) & lon != 0 & lat != 0) %>%
  distinct(lon, lat, .keep_all = TRUE)
write.csv(df_clean, "o1_occs_clean.csv", row.names = FALSE)
...
st_write(m_area, "o1_occs_m_area.geojson", delete_dsn = TRUE, quiet = TRUE)
...
write.csv(cor_mat, "o1_correlation_matrix.csv", row.names = TRUE)

dist_mat <- as.dist(1 - abs(cor_mat))
hc <- hclust(dist_mat, method = "average")

png("o1_variable_dendrogram.png", width = 800, height = 600, res = 100)
...
names(occs_thinned) <- tolower(names(occs_thinned))
...
write.csv(res_table, "o2_calibration_results.csv", row.names = FALSE)
write.csv(res_table, "o4_calibration_results_recalibrated.csv", row.names = FALSE)
...
# Emit metrics
emit_metric("Best Model AUC", res_table$auc.val.avg[best_idx], kind="ratio")
emit_metric("Best Model AICc", res_table$AICc[best_idx], kind="scalar")
...
unzip("barriers_sobralia_turkeliae.zip", exdir = unzip_dir)
...
saveRDS(barriers, "o3_barriers_processed.rds")
saveRDS(barriers, "o8_barriers_processed.rds") # for compatibility
saveRDS(barriers, "databases/o3_barriers_processed.rds") # target location
message("Barriers processed and saved successfully.")

# Copy the manifest
if (file.exists("external_inputs_manifest.json")) {
  file.copy("external_inputs_manifest.json", "o3_external_inputs_manifest.json", overwrite = TRUE)
  file.copy("external_inputs_manifest.json", "analysis-results/o3_external_inputs_manifest.json", overwrite = TRUE)
}
...
write.csv(var_imp, "nod_variable_importance.csv", row.names = FALSE)
...
p_imp <- ggplot(var_imp, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  theme_minimal()
ggsave("nod_variable_importance.png", plot = p_imp, width = 6, height = 4, dpi = 300, bg = "white")

# Plot Response Curves
png("nod_response_curves.png", width = 800, height = 600, res = 100)
...
ggsave("nod_environmental_tolerance.png", plot = p_tol, width = 9, height = 6, dpi = 300, bg = "white")

# -----------------------------------------------------------------------------
# STEP 6: Suitability Projections (Peru, Ecuador, Colombia)
# -----------------------------------------------------------------------------
message("[6/10] Extrapolating suitability model over regional extent...")
countries_sf <- st_read("gadm_4.1_per_ecu_col_level0.gpkg", quiet = TRUE)
...
terra::writeRaster(pred_raster_extrap, "o4_suitability_extrapolated.tif", overwrite = TRUE)
terra::writeRaster(pred_raster_extrap, "o4_suitability_extrapolated.asc", filetype = "AAIGrid", overwrite = TRUE)
...
amazon <- st_read("amazon_basin.shp", quiet = TRUE)
suit_amazon <- terra::crop(pred_raster_extrap, vect(amazon), mask = TRUE)
terra::writeRaster(suit_amazon, "o5_clipped_suitability.tif", overwrite = TRUE)
terra::writeRaster(suit_amazon, "o5_clipped_suitability.asc", filetype = "AAIGrid", overwrite = TRUE)

# Mask with 'mantener'
mantener <- st_read("mantener.geojson", quiet = TRUE)
suit_double_masked <- terra::mask(suit_amazon, vect(mantener))
terra::writeRaster(suit_double_masked, "o8_suitability_double_masked.tif", overwrite = TRUE)
...
terra::writeRaster(binary_suit, "o8_binary_suitability.tif", overwrite = TRUE)
...
eoo_sf_df <- st_sf(geometry = st_sfc(eoo_wgs84))
st_write(eoo_sf_df, "o8_eoo_mcp.geojson", delete_dsn = TRUE, quiet = TRUE)
...
st_write(occupied_cells_wgs84, "o8_aoo_grid.geojson", delete_dsn = TRUE, quiet = TRUE)
...
write.csv(metrics_df, "o8_iucn_metrics.csv", row.names = FALSE)
...
countries_sf <- st_transform(countries_sf, st_crs(suit_double_masked))
...
p_final <- ggplot() +
  ...
  scale_fill_manual(
    name = "Elementos",
    values = c("Presencias" = "red", "EOO" = NA, "AOO" = "blue"),
    breaks = c("Presencias", "EOO", "AOO"),
    guide = guide_legend(
      override.aes = list(
        shape = c(21, NA, NA),
...
ggsave("o3_final_categorized_map.png", plot = p_final, width = 7.5, height = 9.5, dpi = 300, bg = "white")
...
ggsave("o5_final_map_countries_fixed.png", plot = p_countries, width = 8, height = 8, dpi = 300, bg = "white")

message("=== PIPELINE EXECUTION SUCCESSFUL ===")


# ──── #2 · Ejecución · 2026-08-06T15:28:28.605Z ───────────────────────────────────
# Rerun full SDM pipeline with standard coordinates
# Propósito: Execute the complete, unified SDM pipeline for Sobralia turkeliae with synchronized coordinate headers to calibrate the model, calculate metrics, and output final plots cleanly and reproducibly.
# Artifacts: #3911, #3915, #3913, #3912, #3914, #3916, #3917, #3918, #3920, #3919, #3921, #3922
# runtime: modal:sdm-r
# duration_ms: 104057
...
# ──── #3 · Ejecución · 2026-08-06T16:56:12.146Z ───────────────────────────────────
# Verified reproducibility rehabilitation — full SDM rerun
# Propósito: Deterministic full rerun from canonical inputs after fixing raster serialization and causal capture
# error: Error in `[[<-.data.frame`(`*tmp*`, i, value = c(21, NA, NA)) : 
  replacement has 3 rows, data has 2
Calls: <Anonymous> ... <Anonymous> -> modify_list -> [[<- -> [[<-.data.frame
In addition: Warning …
# failed: true
# runtime: modal:sdm-r
# duration_ms: 105648
...
required_inputs <- c(
  "Coordenadas_Sobralia turkeliae_nov.xlsx",
  "barriers_sobralia_turkeliae.zip",
  "amazonas.zip",
  "gadm_4.1_per_ecu_col_level0.gpkg",
  "mantener.geojson",
  "external_inputs_manifest.json"
)
...
# Copy the manifest
if (file.exists("external_inputs_manifest.json")) {
  copied_manifest <- file.copy("external_inputs_manifest.json", "analysis-results/o3_external_inputs_manifest.json", overwrite = TRUE)
...
unzip("amazonas.zip", exdir = amazon_dir)
...
ggsave("nod_final_map_countries_fixed.png", plot = p_countries, width = 8, height = 8, dpi = 300, bg = "white")

required_outputs <- c(
  "analysis-results/o3_external_inputs_manifest.json",
  "databases/o3_barriers_processed.rds",
  "nod_environmental_tolerance.png",
  "nod_final_map_countries_fixed.png",
  "nod_response_curves.png",
  "nod_variable_importance.png",
  "o5_clipped_suitability.tif",
  "o8_suitability_double_masked.tif"
)
...
if (!identical(unname(tools::md5sum("external_inputs_manifest.json")), unname(tools::md5sum("analysis-results/o3_external_inputs_manifest.json")))) stop("Manifest bytes changed during copy")
if (terra::ncell(terra::rast("o5_clipped_suitability.tif")) <= 0) stop("Clipped suitability GeoTIFF is invalid")
if (terra::ncell(terra::rast("o8_suitability_double_masked.tif")) <= 0) stop("Double-masked suitability GeoTIFF is invalid")
message("=== PIPELINE EXECUTION AND OUTPUT VALIDATION SUCCESSFUL ===")


# ──── #4 · Ejecución · 2026-08-06T17:06:32.385Z ───────────────────────────────────
# Verified reproducibility rehabilitation — full SDM rerun
# Propósito: Deterministic full rerun from canonical inputs after fixing raster serialization and causal capture
# Artifacts: #3943, #3944, #3942, #3945, #3946, #3947, #3948, #3949, #3953, #3950, #3951, #3952, #3954, #3955, #3956, #3957, #3958, #3959, #3960, #3961
# runtime: modal:sdm-r
# duration_ms: 198954
...
# ──── #5 · Ejecución · 2026-08-06T18:10:12.595Z ───────────────────────────────────
# Verified reproducibility rehabilitation — full SDM rerun
# Propósito: Deterministic full rerun from canonical inputs after fixing raster serialization and causal capture
# Artifacts: #3966, #3967, #3965, #3968, #3969, #3970, #3971, #3972, #3975, #3974, #3973, #3976, #3977, #3978, #3979, #3980, #3981, #3982, #3983, #3984
# runtime: modal:sdm-r
# duration_ms: 146963
