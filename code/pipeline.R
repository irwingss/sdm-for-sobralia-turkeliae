# Species distribution model for *Sobralia turkeliae* — Unified R Analysis Pipeline
...
.rs_argv <- commandArgs(trailingOnly = FALSE)
.rs_file <- sub('^--file=', '', .rs_argv[grep('^--file=', .rs_argv)])
PACKAGE_ROOT <- if (length(.rs_file) > 0) {
  dirname(dirname(normalizePath(.rs_file[1])))
} else {
  normalizePath(getwd())
}
...
set.seed(570887781)
Sys.setenv(R_SEED = "570887781")

.scalars_out <- file.path(PACKAGE_ROOT, '_local_outputs/scalars_reproduced.jsonl')
dir.create(dirname(.scalars_out), showWarnings = FALSE, recursive = TRUE)
cat('', file = .scalars_out)
Sys.setenv(RESSEARCH_SCALARS_OUT = .scalars_out)
...
# ─── Step 1: Regenerate the two required o3 results from original canonical inputs with embedded deterministic provenance ───────────────────
...
# ─── Step 2: Deterministic full rerun from canonical inputs after fixing raster serialization and causal capture ───────────────────
